...
export class SimulationEngine {
    ...
    constructor(opts) {
        ...
        if (!ctx)
            throw new Error('Canvas 2D를 초기화할 수 없습니다.');
        this.ctx = ctx;
    ...
    t() { return (performance.now() - this.started) / 1000; }
    emit(type, message) {
    ...
    spawn(species, mutate, parentIndex) {
        if (this.nextIndex >= MAX_AGENTS)
            return -1;
        const idx = this.nextIndex++;
        this.writeAgent(idx, species, parentIndex);
        if (mutate)
            this.mutateAgent(idx, false);
        return idx;
    }
    async initGpu() {
        if (!('gpu' in navigator)) {
...
fn main(@builtin(global_invocation_id) gid: vec3u) {
  ...
  agents[i] = a;
    ...
    writeGpu() {
        if (this.gpu)
            this.gpu.device.queue.writeBuffer(this.gpu.agentBuffer, 0, this.data);
    }
    gpuStep(dt) {
        if (!this.gpu)
            return;
    ...
    cpuStep(dt) {
        ...
        for (let i = 0; i < this.nextIndex; i++) {
            ...
            if (this.world.temperature < 0.28)
                drain *= 1.45;
            if (this.world.waterDependency && Math.hypot(x - 0.22, y - 0.72) > 0.24)
                drain += 0.026 * dt;
            if (species < 0.5) {
    ...
    ecologyTick(windowSeconds) {
        ...
        for (let i = 0; i < currentNext && this.nextIndex < MAX_AGENTS; i++) {
            const o = i * STRIDE;
            if (this.data[o + 7] < 0.5 || this.data[o + 4] < 1.12)
                continue;
        ...
        if (births > 20 && this.random() < 0.12)
            this.emit('birth', `새 개체 ${births.toLocaleString()}마리 탄생`);
    ...
    draw() {
        ...
        const grad = ctx.createRadialGradient(this.world.lightX * cw, this.world.lightY * ch, 0, this.world.lightX * cw, this.world.lightY * ch, Math.max(cw, ch) * 0.5);
        ...
        if (this.world.waterDependency) {
            ctx.beginPath();
            ctx.arc(0.22 * cw, 0.72 * ch, Math.min(cw, ch) * 0.19, 0, Math.PI * 2);
        ...
        ctx.fillStyle = 'rgba(168,241,103,.28)';
        for (let i = 0; i < foodDots; i++) {
            const a = ((i * 9301 + 49297) % 233280) / 233280;
            const b = ((i * 2333 + 7919) % 104729) / 104729;
            const x = ((a * 0.82 + this.world.lightX * 0.18) % 1) * cw;
            const y = ((b * 0.82 + this.world.lightY * 0.18) % 1) * ch;
        ...
        for (let i = 0; i < this.nextIndex; i += sampleStep) {
            ...
            if (this.data[o + 7] < 0.5)
                continue;
            const species = this.data[o + 6];
            const x = this.data[o] * cw, y = this.data[o + 1] * ch;
            const size = clamp(this.data[o + 12] * (species < 0.5 ? 1.25 : 1.7), 0.8, 3.4);
            const mut = this.data[o + 15];
            ctx.fillStyle = species < 0.5
                ? `rgba(${Math.round(88 + mut * 180)}, ${Math.round(210 - mut * 90)}, 255, .82)`
                : `rgba(255, ${Math.round(98 + mut * 90)}, ${Math.round(112 - mut * 80)}, .86)`;
    ...
    addPredators(count) { for (let i = 0; i < count && this.nextIndex < MAX_AGENTS; i++)
        this.spawn(1, false); this.writeGpu(); }
    mutateRandom() {
        const candidates = [];
        for (let i = 0; i < this.nextIndex; i += Math.max(1, Math.floor(this.nextIndex / 6000)))
            if (this.data[i * STRIDE + 7] > 0.5)
                candidates.push(i);
